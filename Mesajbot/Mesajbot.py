...
async def Kullanıcıya_Soru_Gönder(channel, user_id):
    ...
    await channel.send(soru.text, view=görüntü)  # burası düzeltildi
...
async def on_message(message):
    ...
    if re.search(r"\s*m+\s*e+\s*r+\s*h+\s*a+\s*b+\s*a+\s*", cleaned_content):
    ...
    elif re.search(r"\s*(n+a+s+ı+l+s+ı+n|n+a+s+i+l+s+i+n|i+y+i\s*m+i+s+i+n)\s*(\?*)\s*", cleaned_content, re.IGNORECASE):
        await message.channel.send("İyiyim, sorduğun için teşekkür ederim.")
        if random.randint(1, 2) == 1:
            await message.add_reaction("👍🏻")

    elif re.search(r"\s*(a+d+|a+d+ı+n?|a+d+i+n?)\s*(\s*n+e)?\s*(\?*)\s*", cleaned_content, re.IGNORECASE):
        await message.channel.send("Benim Adım Mesajbot.")
        if random.randint(1, 2) == 1:
            await message.add_reaction("👍🏻")

    elif re.search(r"\s*(i+s+m+ı+n?|i+s+i+m+)\s*(\s*n+e)?\s*(\?*)\s*", cleaned_content, re.IGNORECASE):
        await message.channel.send("Benim ismim Mesajbot.")
        if random.randint(1, 2) == 1:
            await message.add_reaction("👍🏻")

    elif re.search(r"\s*s+e+n+i+\s*(k+i+m|y+a+p+a+n|y+ı+z+ı+l+m+a+y+a+n|y+ı+z+ı+l+m+a+y+a+n+ı+n)\s*(a+d+ı|a+d+i|i+s+m+i)?\s*(n+e)?\s*\??\s*", cleaned_content, re.IGNORECASE):
        await message.channel.send("Beni Mert NALBANTOĞLU yazılımladı.")
        if random.randint(1, 2) == 1:
            await message.add_reaction("👍🏻")

    elif re.search(r"\s*s+\s*a+\s*n+\s*a+\s*\s*g+\s*u+\s*v+\s*e+\s*n+\s*i+\s*b+\s*i+\s*l+\s*i+\s*r+\s*\s*m+\s*i+\s*y+\s*i+\s*m|\s*s+\s*e+n+\s*\s*g+\s*u+\s*v+\s*e+\s*n+\s*i+\s*l+\s*i+\s*r+\s*m+\s*i+\s*n|\s*g+\s*u+\s*v+\s*e+\s*n+\s*i+\s*l+\s*i+\s*r(\s*m+\s*i+\s*n)?", cleaned_content, re.IGNORECASE):
        await message.channel.send("Evet, bana güvenebilirsin.")
        if random.randint(1, 2) == 1:
            await message.add_reaction("👍🏻")

    elif re.search(r"\s*s+\s*e+\s*n+\s*\s*n+\s*e+\s*s+\s*i+\s*n\s*(\?*)\s*", cleaned_content, re.IGNORECASE):
        await message.channel.send("Ben bir yapay zekayım.")
        if random.randint(1, 2) == 1:
            await message.add_reaction("👍🏻")

    elif re.search(r"\s*t+\s*e+\s*ş+\s*e+\s*k+\s*ü+\s*r+\s*(l+e+r+|e+d+e+r+i+m+)?\s*(\?*)\s*", cleaned_content):
    ...
    elif re.search(r"\s*e+\s*m+\s*o+\s*j+\s*i+(\s*(a+t|g+ö+n+d+e+r|y+o+l+l+a))?\s*(\?*)\s*", cleaned_content):
        await message.channel.send(Emoji_Gönder())
        if random.randint(1, 2) == 1:
            await message.add_reaction("👍🏻")

    elif re.search(r"s+o+r+u*r*|s+o*r*|s+o+r+u+\s*s+o+r*|s+o+r+u+l+a*r*|s+o*r+u+l+a*r\s+s+o*r*", cleaned_content, re.IGNORECASE):
        ...
        await message.channel.send("Mesajı arapçaya çevirmek için lütfen aşağıdaki düğmeye basınız.", view=görüntü)  # burası düzeltildi

    else:
        await message.channel.send("Mesaj Anlaşılamadı.")
